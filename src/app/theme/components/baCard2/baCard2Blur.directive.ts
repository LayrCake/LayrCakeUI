...
import {BaCard2BlurHelper} from './baCard2BlurHelper.service';
import {BgMetrics} from './bgMetrics';

@Directive({
  selector: '[baCardBlur]',
  providers: [BaCard2BlurHelper]
})
export class BaCard2Blur {

  @HostBinding('class.card-blur') isEnabled:boolean = false;

  private _bodyBgSize:BgMetrics;

  constructor(private _baConfig:BaThemeConfigProvider, private _baCardBlurHelper:BaCard2BlurHelper, private _el:ElementRef) {
    if (this._isEnabled()) {
